...
This project is analyzed on every push to `main` via [SonarCloud](https://sonarcloud.io), integrated through a GitHub Actions workflow (`.github/workflows/build.yml`).

**What the pipeline does:**
1. Runs the backend test suite (Mocha/Chai) with coverage via `nyc`
2. Runs the frontend test suite (Jest) with coverage
3. Uploads both `lcov.info` reports to SonarCloud for combined analysis
...
Screenshots of the full analysis are available in [`notes-app/sonarcubeReport/`](./notes-app/sonarcubeReport).
